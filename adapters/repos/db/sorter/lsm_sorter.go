...
func newLSMStoreSorter(store *lsmkv.Store, schema schema.Schema, className schema.ClassName, property, order string) *lsmSorter {
	return &lsmSorter{
		store:             store,
		className:         className,
		sorterClassHelper: sorterClassHelper{schema},
		property:          property,
		order:             order,
		sorter:            sortBy{comparator{order}},
		propertyExtractor: &lsmPropertyExtractor{className, &sorterClassHelper{schema}, property},
	}
}
...
func (s *lsmSorter) currentIsBetterThanWorstElement(candidates []docIDAndValue, limit int,
	curr interface{}, order string) bool {
...
func (s *lsmSorter) addToCandidates(candidates []docIDAndValue, limit int,
	newEntry docIDAndValue, order string) []docIDAndValue {
	if len(candidates) == 0 {
...
func (s *lsmSorter) compare(curr, target interface{}) bool {
	...
	if len(dataType) > 0 {
		return s.sorter.lessBy(curr, target, schema.DataType(dataType[0]))
	}
	return false
}
